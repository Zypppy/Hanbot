...
local common = module.load("ZypppyMorgana", "common")
...
local menu = menu("ZypppyMorgana", "Morgana By Zypppy")
menu:menu("c", "Combo")
menu.c:boolean("qcombo", "Use Q in Combo", true)
--menu.c:dropdown("qmode", "Q Prediction", 1, {"Fast Prediction", "Slow Prediction"})
--menu.c:boolean("qslow", "Q Slow Prediction", false)
...
for i = 1, #common.GetEnemyHeroes() do
	...
	if enemy and interruptableSpells[name] then
		for v = 1, #interruptableSpells[name] do
			...
			menu.misc.setq.interruptmenuq:boolean(
...
if menu.misc.seq.Qinterr:get() and player:spellSlot(0).state == 0 then
   ...
   local enemyName = string.lower(spell.owner.charName)
      if interruptableSpells[enemyName] then
		 ...
		 local spellCheck = interruptableSpells[enemyName][i]
			 ...
			 if player.pos2D:dist(spell.owner.pos2D) <= spellQ.range and common.IsValidTarget(spell.owner) and 
...
if menu.c.qcombo:get() and common.IsValidTarget(target) then
local target = GetTargetQ()
...
if menu.c.wcombo:get() and common.IsValidTarget(target) then
...
local function AutoDash()
	...
	local target =
		...
		function(res, obj, dist)
			if dist <= spellQ.range and obj.path.isActive and obj.path.isDashing and not common.CheckBuffType(enemies, 17) then
				res.obj = obj
				return true
			end
		end
